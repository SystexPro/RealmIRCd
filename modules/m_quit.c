...
DLLFUNC int  m_quit(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	if (!IsServer(cptr) && IsPerson(sptr))
	{
		...
		if (!prefix_quit || strcmp(prefix_quit, "no"))
			/* Added a custom quit for Op3r's -cdh */
			if(IsOper(sptr)) {
				s = ircsprintf(comment, "%s ", BadPtr(prefix_quit) ? "Oper Quit:" : prefix_quit);/* Custom? */
			}
			s = ircsprintf(comment, "%s ", BadPtr(prefix_quit) ? "Quit:" : prefix_quit);
#ifdef STRIPBADWORDS
